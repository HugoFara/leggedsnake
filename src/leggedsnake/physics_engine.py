...
class World:
    ...
    linkages: list[dynamic_linkage.DynamicLinkage]
    config: WorldConfig
    ...
    def add_linkage(
        ...
        """
        if isinstance(source, dynamic_linkage.DynamicLinkage):
            dl = source
        else:
            load_mass = self.config.load_mass if load is None else load
            dl = dynamic_linkage.convert_to_dynamic_linkage(
    ...
    def _apply_ground_filter(self) -> None:
        ...
        """
        from . import dynamic_linkage as _dl
    ...
    def __update_linkage__(
        self, linkage: dynamic_linkage.DynamicLinkage, power: float
    ) -> tuple[float, float]:
...
def recalc_linkage(linkage: dynamic_linkage.DynamicLinkage) -> None:
